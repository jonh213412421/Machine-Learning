...
with st.sidebar.expander("Parâmetros avançados"):
    st.session_state.funcao_ativacao = st.selectbox("Escolha a função de ativação", options=["relu", "tanh", "sigmoid", "leakyrelu", "linear"])
    st.session_state.aprendizado = st.select_slider("Taxa de Aprendizado", [0.001, 0.01, 0.1], value=0.01)
    epochs = st.slider("Quantidade de Eras de Treinamento", 100, 10000, value=2500)
    tamanho_rede = st.slider("Número de Nodes na Rede", 5, 50, value=10)
    st.button("Reconfigurar Modelo", on_click=criar_modelo)
...
if st.session_state.iniciado:
    ...
    if not st.session_state.modelo_treinado:
        ...
        while st.session_state.indice_epoca < st.session_state.epochs:
            parar = st.session_state.parar
            st.session_state.estado_treinamento = RedeNeural.fit(st.session_state.modelo, coordenadas, epochs, tamanho_rede, st.session_state.indice_epoca, parar, plot_placeholder, progress_bar, status_placeholder)
            print(f"y_pred: {st.session_state.estado_treinamento}")
            print(parar)

            st.session_state.indice_epoca = st.session_state.indice_epoca + 1

            if st.session_state.indice_epoca == st.session_state.epochs:
                st.session_state.modelo_treinado = True
